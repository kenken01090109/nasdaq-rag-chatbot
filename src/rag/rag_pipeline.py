...
class RAGPipeline:
    def __init__(
        ...
        print(f"DEBUG raw docs dir: {self.raw_docs_dir}")
        print(f"DEBUG chroma dir: {self.chroma_dir}")
        ...
        print(f"DEBUG final collection count: {self.collection.count() if not self.collection_is_empty else 0}")
    ...
    def _initialize_collection_if_empty(self) -> None:
        try:
            ...
            print(f"DEBUG existing collection count before init: {current_count}")
            if current_count > 0:
                return
        except Exception as e:
            print(f"DEBUG count check failed before init: {e}")
        ...
        if not txt_files:
            print("DEBUG no raw txt docs found")
            return
            ...
            for idx, chunk in enumerate(chunks):
                ...
                if len(ids_batch) >= batch_size:
                    try:
                        ...
                        print(f"DEBUG batch add failed: {e}")
        ...
        if ids_batch:
            try:
                ...
                print(f"DEBUG final batch add failed: {e}")

        try:
            print(f"DEBUG Chroma initialization complete. Total chunks added: {total_chunks}")
            print(f"DEBUG collection count after init: {self.collection.count()}")
        except Exception as e:
            print(f"DEBUG post-init count failed: {e}")
...
""".strip()
    ...
    def _raw_vector_search(
        ...
        try:
            ...
            print(f"DEBUG collection query failed: {e}")
            return []
...
""".strip()
    ...
    def answer(
        ...
        try:
            ...
            answer_text = f"Model generation failed: {e}"
